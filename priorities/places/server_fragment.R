...
median_resident_earnings <-
  read_csv("data/places/median_resident_earnings.csv") %>%
  mutate(
    area_name = factor(
      area_name,
      levels = c("Trafford", "Greater Manchester", "England"),
...
output$claimant_count_plot <- renderggiraph({
  gg <-
    ...
    geom_line_interactive(aes(linetype = group, tooltip = tooltip),
                          size = 1,
                          colour = "#00AFBB") +
    scale_y_continuous(limits = c(0, NA), labels = comma) +
    labs(
  ...
  gg <- girafe(ggobj = gg)
  tooltip_css <-
    "background-color:#00AFBB;color:#FFFFFF;padding:0.3em;"
  girafe_options(gg,
...
output$apprenticeships_plot <- renderggiraph({
  if (input$apprenticeships_selection == "GM boroughs") {
    gg <-
      ggplot(apprenticeships, aes(x = period, y = value, fill = area_name)) +
...
real_living_wage <-
  read_csv("data/places/real_living_wage.csv") %>%
...
gva <- read_csv("data/places/gva.csv") %>%
  mutate(
    group = fct_relevel(group, "All industries"),
    tooltip =
      paste0(
        "<strong>",
        "£",
...
output$gva_plot <- renderggiraph({
  gg <- ggplot(filter(gva, group %in% input$gva_selection),
    ...
    scale_y_continuous(limits = c(0, NA), labels = comma) +
    labs(
      title = "Total GVA by industry sector in 2016",
