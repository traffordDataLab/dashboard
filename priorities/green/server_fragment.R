...
gmal <- st_read("data/green/gmal.geojson")
  ...
  green_flags <- read_csv("data/green/green_flags.csv") %>%
    mutate(area_name = as_factor(area_name))
  ...
  output$green_flags_map = renderLeaflet({
    if (input$green_flags_selection == "Greater Manchester") {
      ...
      leaflet() %>%
        ...
        addCircleMarkers(
          data = filter(green_flags, area_name == "Trafford"),
          lng = ~ lon,
          lat = ~ lat,
          stroke = TRUE,
          color = "#212121",
          ...
          popup = paste(
            "<strong>",
            filter(green_flags, area_name == "Trafford")$name,
            "</strong><br />",
            "<em>",
            filter(green_flags, area_name == "Trafford")$area_name,
            "</em><br/>",
            "<a href='",
            filter(green_flags, area_name == "Trafford")$url,
            "' target='_blank'>Further info</a>"
          )
        )
    }
    ...
    div(class = "col-sm-12 col-md-6 col-lg-4",
        box(
          ...
          div(
            style = "position: absolute; left: 4em; bottom: 0.5em;",
            dropdown(
              includeMarkdown("data/green/metadata/green_flags.md"),
              icon = icon("question"),
              size = "xs",
              style = "jelly",
              width = "300px",
